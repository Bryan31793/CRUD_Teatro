...
public class VentaBoletos extends JFrame {
    ...
    public VentaBoletos(int idEmpleadoLogueado) {
        ...
        // --- Aplicar Renderer Personalizado para Visibilidad ---
        setCustomRenderer(cmbFuncion);
        setCustomRenderer(cmbCliente);
        setCustomRenderer(cmbMetodoPago);
...
private JComboBox<ComboBoxItem> crearComboBox(int x, int y, int width) {
        ...
        // --- Solución: Forzamos el color del texto a negro ---
        // Dejamos que el LAF pinte el fondo en blanco/gris (como en tu imagen)
        cmb.setForeground(Color.BLACK); 
        
        // El setBackground(COLOR_CARD) se ha removido para evitar la sobreescritura
        // y dejar el color blanco que el LAF insiste en usar.

        // --- 2. RENDERER PERSONALIZADO (Asegura que la lista desplegada SÍ sea oscura) ---
        cmb.setRenderer(new DefaultListCellRenderer() {
            // Re-declaramos los colores para usarlos en el Renderer (lista que se abre)
            private final Color COLOR_PANEL = new Color(45, 55, 100);
            ...
            public Component getListCellRendererComponent(JList<?> list, Object value, int index, 
                ...
                // Si index == -1, es la caja principal, ya la arreglamos con cmb.setForeground(Color.BLACK)
                if (index > -1) {
                    // Si es la lista desplegada: Usar tema oscuro para las opciones
                    if (!isSelected) { 
                        label.setForeground(COLOR_TEXTO); // Letra blanca
                        label.setBackground(COLOR_PANEL); // Fondo oscuro
                    } else { // Item seleccionado (hover) en la lista
                        label.setForeground(Color.BLACK); // Letra negra en selección
                        label.setBackground(COLOR_DORADO); 
                    }
                }
                
                // Si el valor no es nulo, usa el texto del ComboBoxItem
                if (value instanceof ComboBoxItem) {
                    label.setText(((ComboBoxItem) value).getTexto());
                } else if (value == null) {
    ...
    /**
     * Aplica un renderer personalizado a un JComboBox.
     * Corrige el problema de la letra blanca en el fondo blanco y aplica el tema oscuro a la lista.
     */
    private void setCustomRenderer(JComboBox<ComboBoxItem> cmb) {
        cmb.setRenderer(new DefaultListCellRenderer() {
            // Colores del tema para referencia interna en el renderer
            private final Color COLOR_PANEL = new Color(45, 55, 100);
            private final Color COLOR_TEXTO = Color.WHITE;
            private final Color COLOR_DORADO = new Color(255, 193, 7);
            ...
            public Component getListCellRendererComponent(JList<?> list, Object value, int index, 
                ...
                // --- Aplicar Tema Oscuro a la Lista ---
                if (!isSelected) {
                    // Item no seleccionado: Fondo oscuro, Letra blanca
                    label.setForeground(COLOR_TEXTO); 
                    label.setBackground(COLOR_PANEL); 
                } else {
                    // Item seleccionado (hover): Fondo dorado, Letra oscura
                ...
                if (value instanceof ComboBoxItem) {
                    ...
                    // Manejar el caso de que el JComboBox esté vacío
                    label.setText("— Seleccione —"); 
                }
