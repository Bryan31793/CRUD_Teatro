    ...
    private void cargarDatosIniciales() {
        ...
        try {
            ...
            PreparedStatement pst = con.prepareStatement(
                "SELECT id_funcion, CONCAT(titulo_funcion, ' - ', DATE_FORMAT(fecha_fun, '%d/%m/%Y'), ' ', TIME_FORMAT(hora_fun, '%H:%i')) AS desc_funcion " +
                "FROM Funcion WHERE fecha_fun >= CURDATE() ORDER BY fecha_fun, hora_fun"
            );
            ResultSet rs = pst.executeQuery();
            while (rs.next()) {
                cmbFuncion.addItem(new ComboBoxItem(rs.getInt("id_funcion"), rs.getString("desc_funcion")));
            }
            rs.close();
            pst.close();

            // Cargar Clientes
            pst = con.prepareStatement("SELECT id_cliente, nombre_cliente FROM Cliente ORDER BY nombre_cliente");
            rs = pst.executeQuery();
            while (rs.next()) {
            ...
            pst = con.prepareStatement("SELECT id_metodoPago, tipo FROM MetodoPago");
            rs = pst.executeQuery();
            while (rs.next()) {
