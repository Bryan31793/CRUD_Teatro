...
// --- NOTA: Debes asegurar que las variables COLOR_DORADO, COLOR_TEXTO, COLOR_CARD, etc.,
//           estén definidas al inicio de tu clase AdminFunciones.java
//           (Estas variables se asumen que son las mismas que en VentaBoletos.java)
...
private JComboBox<ComboBoxItem> crearComboBox(int x, int y, int width) {
    ...
    // --- 1. SOLUCIÓN: FORZAR TEXTO NEGRO Y QUITAR OVERRIDE DE COLOR ---
    // Dejamos que el LAF pinte el fondo en blanco/gris (como en tu imagen)
    cmb.setBackground(new Color(255, 255, 255)); // Dejamos el fondo en blanco/claro
    cmb.setForeground(Color.BLACK); // <-- Aseguramos texto negro en la caja principal
    cmb.setOpaque(true);

    // --- 2. RENDERER PERSONALIZADO (Para la lista que se abre) ---
    cmb.setRenderer(new DefaultListCellRenderer() {
        private final Color COLOR_PANEL_REF = new Color(45, 55, 100);
        private final Color COLOR_TEXTO_REF = Color.WHITE;
        private final Color COLOR_DORADO_REF = new Color(255, 193, 7);
        ...
        public Component getListCellRendererComponent(JList<?> list, Object value, int index, 
            ...
            // --- Si index == -1, estamos pintando la caja principal. Si index > -1, la lista ---
            
            if (index > -1) {
                // Si es la lista desplegada: Usar tema oscuro para las opciones
                if (!isSelected) { 
                    label.setForeground(COLOR_TEXTO_REF); // Letra blanca
                    label.setBackground(COLOR_PANEL_REF); // Fondo oscuro
                } else { // Item seleccionado (hover) en la lista
                    label.setForeground(Color.BLACK); // Letra negra en selección
                    label.setBackground(COLOR_DORADO_REF); 
                }
            }
