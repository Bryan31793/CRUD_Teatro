    ...
    private void cargarDatosComboBox() {
        ...
        try {
            ...
            PreparedStatement pst = con.prepareStatement(
                "SELECT id_funcion, CONCAT(titulo_funcion, ' - ', DATE_FORMAT(fecha_fun, '%d/%m/%Y')) AS desc_funcion " +
                "FROM Funcion ORDER BY fecha_fun DESC"
            );
            ResultSet rs = pst.executeQuery();
            while (rs.next()) {
                cmbFuncionesExistentes.addItem(new ComboBoxItem(rs.getInt("id_funcion"), rs.getString("desc_funcion")));
            }
            rs.close();
            pst.close();

            // Cargar Zonas
            pst = con.prepareStatement("SELECT id_zona, CONCAT(nombre_zona, ' (Sala: ', (SELECT nombre_sala FROM Sala WHERE id_sala = Zona.Sala_id_sala), ')') AS desc_zona FROM Zona");
            rs = pst.executeQuery();
            while (rs.next()) {
                cmbZonas.addItem(new ComboBoxItem(rs.getInt("id_zona"), rs.getString("desc_zona")));
            }
            rs.close();
            pst.close();

            // Cargar Salas
            pst = con.prepareStatement("SELECT id_sala, CONCAT(nombre_sala, ' (Cap: ', capacidad, ')') AS desc_sala FROM Sala");
            rs = pst.executeQuery();
            while (rs.next()) {
                cmbSala.addItem(new ComboBoxItem(rs.getInt("id_sala"), rs.getString("desc_sala")));
            }
            rs.close();
            pst.close();
